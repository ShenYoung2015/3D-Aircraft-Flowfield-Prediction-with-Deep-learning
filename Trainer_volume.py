...
class Trainer:
    ...
    def eval_write_image(self):
        ...
        #                              shuffle=False)                       
        # idx = 1
        # input_data = eval_dataloader.dataset[idx]
        # xyzn_surf_source, xyzn_surf, xyz_volume_ds, gts_surf, gts_vol = input_data
        # xyzn_surf, xyz_volume_ds = xyzn_surf.unsqueeze(0).to(device), xyz_volume_ds.unsqueeze(0).to(device)
        # xyzn_surf_source = xyzn_surf_source.unsqueeze(0).to(device)
        ...
        xyzn_surf_source = xyzn_surf.to(device)
        ## custom eval ends

        x_points = torch.linspace(xyzn_surf_source[0, 0, :].min().to('cpu')*2, xyzn_surf_source[0, 0, :].max().to('cpu')*2, 500)
        z_points = torch.linspace(xyzn_surf_source[0, 2, :].min().to('cpu')*2, xyzn_surf_source[0, 2, :].max().to('cpu')*2, 300)
        ...
        # Pred
        with torch.no_grad():
            pred = self.model(xyzn_surf_source.to(device).contiguous(), xyz_volume.to(device).contiguous()).squeeze(0)
        pred = np.array(pred.squeeze(0).to('cpu'))
        # pred[pred>0.2] = 0.2
        ...
        # 添加红色散点图
        fig.add_trace(go.Scatter3d(
            x=xyzn_surf_source[0, 0, :].to('cpu'),
            y=xyzn_surf_source[0, 1, :].to('cpu'),
            z=xyzn_surf_source[0, 2, :].to('cpu'),
            mode='markers',
            marker=dict(
                size=0.5,
                color='red',
        ...
        fig.write_image(f'temp_image//{self.epoch}.png')
