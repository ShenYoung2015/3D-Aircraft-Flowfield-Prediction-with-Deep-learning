...
from Trainer_volume import Trainer
...
def main():
    ...
    if args.dataset_path is None or args.dataset_path == 'default':
        if args.dataset == 'SS2000':
            default_dataset_path_list = [r'/root/autodl-tmp/shapesffd3', r'D:\database\shapesffd3']
        else:
            raise ValueError
    ...
    model = PointAR(att_cfg if att_cfg is not None else [256,128,64]).to(device=args.device)
    ...
    scheduler = Scheduler(optimizer, T_max=args.num_epochs, eta_min=args.lr * 0.001)# eta_min=args.lr * 0.001
    # scheduler = torch.optim.lr_scheduler.StepLR(optimizer=optimizer, step_size=10, gamma=0.9)
    # scheduler = scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer=optimizer,T_0=71,T_mult=2)
    # scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer=optimizer, gamma=0.975, last_epoch=-1, verbose=False)
    if sampler is not None:
        optimizer_s = Optimizer(sampler.parameters(), lr=args.lr, weight_decay=args.wd)
        scheduler_s = Scheduler(optimizer_s, T_max=args.num_epochs, eta_min=args.lr/1e6)
    else:
        optimizer_s = None
        scheduler_s = None
    # criterion = weighted_focal_mse_loss().to(args.device)
