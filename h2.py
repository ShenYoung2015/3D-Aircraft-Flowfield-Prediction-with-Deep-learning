...
class att_phase(nn.Module):
    def __init__(self, map_channels=64, number_group=32, group_size=2, dropout=None, h=4):
        ...
        self.after_conv = MLPs(map_channels//self.h, mlp_after_conv, NORM=nn.BatchNorm2d, act=nn.LeakyReLU, dropout=self.dropout, dim=2, drop_last_act=True)# nn.BatchNorm2d , drop_last_act=True
...
class PointAR(nn.Module):
    ...
    def forward(self, xyzn_surf_source, xyz_volume):
        ...
        surf_fps = farthest_point_sample(H=surf_H, npoint=fps_npt*len(self.vol_att), xyz_surf=xyzn_surf_source, wid=round(8*N_surf_source/2048))[1]# [B,FPS,H,wid]
        surf_fps_size = [surf_fps.shape[0], len(self.number_groups),fps_npt, surf_fps.shape[-2], surf_fps.shape[-1]]
        surf_fps = surf_fps.reshape(surf_fps_size) # [B,N-att,fps,H,wid], surf_fps(new)[0,1,0,0,:]==surf_fps[0,512,0,:]
        surf_fps = [surf_fps[:,i].unsqueeze(1) for i in range(len(self.vol_att))]# list: [B,1(C),fps,H,wid]
        ...
        for i in range(len(self.vol_att)):
            ...
            # x1_surf = x1_surf
            # surf_k_source = surf_k_source.reshape(batch_size,-1,surf_H,N_surf_source)
            ...
            l1_surf = torch.gather(x1_surf.unsqueeze(2).expand(-1,-1,fps_npt,-1,-1), dim=-1,index=surf_fps[i].expand(-1,x1_surf.shape[1],-1,-1,-1))
            l1_surf = l1_surf.mean(dim=-1).permute(0,3,2,1).contiguous()# [B,H,fps,C]
            # att-1 mul
            l1_surf = surf_att(global_feat=l1_surf, surf_k_source=surf_k_source.permute(0,2,1,3), x1_surf=x1_surf.permute(0,2,3,1))[0] + l1_surf
            # att-2 mul, in [B,N,C]
            ...
            x1_vol_tmp = self.vol_att[i](x1_surf=l1_surf, x1_q=x1_vol.permute(0, 2, 1))
