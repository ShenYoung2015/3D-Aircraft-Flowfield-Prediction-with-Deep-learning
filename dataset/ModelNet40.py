...
class ModelNet40(Dataset):
    def __init__(self, dataroot, transforms=None):
        ...
        self.train_set = list(glob(os.path.join(self.dataroot, 'train', '*.txt')))#
        # self.train_set = list(glob(os.path.join(self.dataroot, 'test', '*.npz')))
        self.test_set = list(glob(os.path.join(self.dataroot, 'test', '*.txt')))
        self.training = True
    ...
    def __getitem__(self, index):
        # index = 0
        file_name = self.pcd[index]
        label = os.path.split(file_name)[1].split('_')[0]
